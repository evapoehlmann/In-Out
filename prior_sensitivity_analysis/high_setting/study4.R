...
df <- readxl::read_xlsx(tf) |> 
  ...
  mutate(
    ...
    direction = ifelse(item%%2, .5, -.5),
    liking_norm = (Y - 6) / 5
  ) |> 
  dplyr::select(sub, direction, liking_norm)
unlink(tf)

data = list(
  N = nrow(df),
  k = length(unique(df$sub)),
  id = df$sub,
  cond = df$direction,
  y = df$liking_norm
)
...
results[1,4] <- bridgesampling::bayes_factor(ml_unconstrained, ml_common, log = T)$bf
...
# Calculate Log Bayes Factor
results[2,4] <- (log(prior_prob) - log(post_prob))
