...
df <-  read.csv("https://osf.io/download/gcf9s/",
                     sep = ";") |> 
...
results[1,15] <- bridgesampling::bayes_factor(ml_unconstrained, ml_common, log = T)$bf
...
# Calculate Log Bayes Factor
results[2,15] <- (log(prior_prob) - log(post_prob))
